$(document).ready(function () {
    $('#formForgotPassword').on('submit', function (e) {
        ...
        btnEnviar.html('<div class="button-with-icon d-flex justify-content-center">' +
            ...
            '<p class="button-text ms-2" style="text-transform: capitalize;">Enviando...</p>' +
            '</div>');
            ...
            success: function (data) {
                ...
                /* let successHtml = `
                    <div class="alert alert-success alert-dismissible fade show" role="alert">
                        Se ha enviado un enlace a tu correo si está registrado.
            ...
            error: function (jqXHR) {
                ...
                if (response && response.errors) {
                    $.each(response.errors, function (key, messages) {
                        alertHtml += `<li>${messages.join(', ')}</li>`;
                    });
                } else {
                    alertHtml += '<li>Ha ocurrido un error inesperado.</li>';
                }
